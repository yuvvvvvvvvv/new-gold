...
class RiskManager:
    def __init__(self):
        ...
        self.drawdown_levels = {
            ...
            'reduced': 0.10,  # 10% drawdown
            'stopped': 0.15   # 15% drawdown
        }
    ...
    def calculate_position_size(self, account_info, entry_price, stop_loss, correlation_data=None):
        ...
        try:
            ...
            position_size = max(min(position_size, MAX_POSITION_SIZE), 0.01)
    ...
    def fetch_correlation_data(self) -> Dict:
        """Fetch and calculate correlation between gold and USD index"""
        try:
            # Fetch USD index data (DXY)
            usd_rates = mt5.copy_rates_from_pos('USDX', mt5.TIMEFRAME_D1, 0, self.correlation_lookback)
            if usd_rates is None:
                return {}
            
            # Fetch gold data
            gold_rates = mt5.copy_rates_from_pos(SYMBOL, mt5.TIMEFRAME_D1, 0, self.correlation_lookback)
