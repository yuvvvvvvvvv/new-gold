...
class StrategyEngine:
    ...
    def detect_breakout_signals(self, df):
        """Enhanced breakout detection using dynamic Fibonacci levels and harmonic patterns"""
        try:
            signals = []
            ...
            # Detect Fibonacci pattern breakouts
            for level, price in fib_levels.items():
                # Strong breakout with momentum
                if prev_price < price < current_price and df['volume'].iloc[-1] > df['volume'].mean():
                    signals.append(('BREAKOUT', 'LONG', f'FIB_{level}'))
                elif prev_price > price > current_price and df['volume'].iloc[-1] > df['volume'].mean():
    ...
    def is_gartley_pattern(self, swings):
        """Check if swing points form a Gartley pattern"""
        if len(swings) < 5:
            return False
        ...
        return (0.618 <= ab_retracement <= 0.618) and \
               (0.382 <= bc_retracement <= 0.886) and \
               (0.786 <= xd_retracement <= 0.786)
               
    def is_butterfly_pattern(self, swings):
        """Check if swing points form a Butterfly pattern"""
        if len(swings) < 5:
            return False
        ...
        return (0.786 <= ab_retracement <= 0.786) and \
               (0.382 <= bc_retracement <= 0.886) and \
               (1.618 <= xd_retracement <= 2.618)

    def detect_seasonal_patterns(self, df):
        """Identify profitable day-of-week and hour-of-day patterns"""
        try:
            signals = []
    ...
    def combine_strategies_by_regime(self, df, technical_signals, order_flow_signals, ai_signals):
        """Combine strategies with weights based on market regime"""
        try:
